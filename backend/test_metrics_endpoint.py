...
if count == 0:
    ...
    test_incidents = [
        ("INC-001", "jira", "SEV1", "Production outage", base_time, base_time + timedelta(hours=2), "alice"),
        ("INC-002", "jira", "SEV2", "Database slow", base_time + timedelta(days=1), base_time + timedelta(days=1, hours=4), "bob"),
        ("INC-003", "slack", "SEV3", "Minor issue", base_time + timedelta(days=2), base_time + timedelta(days=2, hours=1), "alice"),
        ("INC-004", "jira", "SEV1", "Critical failure", base_time + timedelta(days=5), base_time + timedelta(days=5, hours=3), "charlie"),
        ("INC-005", "slack", "SEV4", "Low priority", base_time + timedelta(days=7), base_time + timedelta(days=8), "bob"),
    ]
    ...
    for ext_id, source, severity, title, occurred, resolved, assignee in test_incidents:
        ...
        """, (
            ext_id, source, severity, title, title,
            occurred.isoformat(), resolved.isoformat(), assignee, "{}"
        ))
...
print(f"SEV1: {metrics.sev1_count}, SEV2: {metrics.sev2_count}, SEV3: {metrics.sev3_count}, SEV4: {metrics.sev4_count}")
print(f"MTTR: {metrics.mean_resolution_hours:.2f}h" if metrics.mean_resolution_hours else "MTTR: N/A")
